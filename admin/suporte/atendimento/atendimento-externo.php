<?php 
	...
	if ( isset( $_POST[ 'relatorio-pdf' ] ) ) {
		...
		echo 'window.open("painel.php?execute=suporte/relatorio/relatorio-atendimento-externo-pdf");';
		echo '</script>';
	}
	...
	if ( isset( $_POST[ 'relatorio-excel' ] ) ) {
		...
		header( 'Location: ../admin/suporte/relatorio/relatorio-atendimento-externo-excel.php' );
	}


	$total = conta('antendimento_externo',"WHERE id AND data_solicitacao>='$data1' AND data_solicitacao<='$data2' ORDER BY data_solicitacao ASC ");

	$leitura = read('antendimento_externo',"WHERE id AND data_solicitacao>='$data1' AND data_solicitacao<='$data2' ORDER BY data_solicitacao DESC, hora_solicitacao DESC");
	...
	if(isset($_POST['pesquisar'])){
		...
		$total = conta('antendimento_externo',"WHERE id AND data_solicitacao>='$data1' AND data_solicitacao<='$data2' ORDER BY data_solicitacao ASC");
		$leitura = read('antendimento_externo',"WHERE id AND data_solicitacao>='$data1' AND data_solicitacao<='$data2' ORDER BY data_solicitacao ASC");
		
	}
 
	if(!empty($status)){
		$total = conta('antendimento_externo',"WHERE id AND data_solicitacao>='$data1' AND data_solicitacao<='$data2' AND status='$status'");
		$leitura = read('antendimento_externo',"WHERE id AND data_solicitacao>='$data1' AND data_solicitacao<='$data2' AND status='$status' ORDER BY data_solicitacao ASC");
	}
 
	if(!empty($suporte)){
		$total = conta('antendimento_externo',"WHERE id AND data_solicitacao>='$data1' AND data_solicitacao<='$data2' AND id_suporte='$suporte'");
		$leitura = read('antendimento_externo',"WHERE id AND data_solicitacao>='$data1' AND data_solicitacao<='$data2' AND id_suporte='$suporte' ORDER BY data_solicitacao ASC");
	}
		
	if(!empty($status) && !empty($suporte ) ){
		$total = conta('antendimento_externo',"WHERE id AND data_solicitacao>='$data1' AND data_solicitacao<='$data2' 
			AND status='$status' AND id_suporte='$suporte'");
		$leitura = read('antendimento_externo',"WHERE id AND data_solicitacao>='$data1' AND data_solicitacao<='$data2'
   ...
   <div class="col-md-12">  
     <div class="box box-default">
		   ...
		   <div class="box-header">
				...
				<a href="painel.php?execute=suporte/atendimento/atendimento-externo-editar" class="btnnovo">
	...
	if($leitura){
		...
		foreach($leitura as $mostra):
		 	echo '<tr>';
				...
				<a href="painel.php?execute=suporte/atendimento/atendimento-externo-editar&suporteEditar='.$mostra['id'].'">
			  				<img src="ico/editar.png" title="Editar" />
              			</a>
				      </td>';
		
				echo '<td align="center">
				<a href="painel.php?execute=suporte/atendimento/atendimento-externo-editar&suporteBaixar='.$mostra['id'].'">
