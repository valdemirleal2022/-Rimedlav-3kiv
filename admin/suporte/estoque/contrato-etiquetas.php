 ...
 <?php 
	...
	if(isset($_POST['relatorio-pdf'])){
		...
		echo 'window.open("painel.php?execute=suporte/relatorio/relatorio-etiqueta-contrato-pdf");';
		echo '</script>';
	}
	...
	if(isset($_POST['relatorio-excel'])){
		$_SESSION['inicio']=$_POST['inicio'];
		$_SESSION['fim']=$_POST['fim'];
	    header('Location: ../admin/suporte/relatorio/relatorio-etiqueta-contrato-excel.php');
	}

	$total = conta('contrato',"WHERE id AND tipo='2' AND status='5' 
			AND saldo_etiqueta_minimo>'0'");

	$leitura = read('contrato',"WHERE id AND tipo='2' AND status='5'  AND saldo_etiqueta_minimo>'0' ORDER BY saldo_etiqueta ASC");
