<?php
	...
	$total = conta('estoque_equipamento_retirada',"WHERE id AND bom_estado='2'");
	$leitura = read('estoque_equipamento_retirada',"WHERE id AND bom_estado='2' ORDER BY data_solicitacao ASC");
	...
	if ( isset( $_POST[ 'relatorio-pdf' ] ) ) {
		...
		echo 'window.open("painel.php?execute=suporte/relatorio/relatorio-equipamento-descarte-pdf");';
		echo '</script>';
	}
	...
	if ( isset( $_POST[ 'relatorio-excel' ] ) ) {
		...
		$_SESSION['fim']=$data2;
		$_SESSION['tipo']=$tipo;
		header( 'Location: ../admin/suporte/relatorio/relatorio-equipamento-descarte-excel.php' );
	}
	...
	if ( isset( $_POST[ 'pesquisar' ] ) ) {
		...
		$total = conta('estoque_equipamento_retirada',"WHERE id AND data_solicitacao>='$data1' 
					 AND data_solicitacao<='$data2' AND bom_estado='2''");
		$leitura = read('estoque_equipamento_retirada',"WHERE id AND data_solicitacao>='$data1' 
					 AND data_solicitacao<='$data2' AND bom_estado='2' ORDER BY data_solicitacao DESC");
	}
