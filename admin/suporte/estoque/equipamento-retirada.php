<?php
	...
	$total = conta('estoque_equipamento_retirada',"WHERE id AND status='Baixado'");
	$leitura = read('estoque_equipamento_retirada',"WHERE id AND status='Baixado' ORDER BY data_solicitacao DESC LIMIT $inicio,$maximo");
	...
	if ( isset( $_POST[ 'pesquisar' ] ) ) {
		...
		$_SESSION['fim']=$data2;
		$_SESSION['tipo']=$tipo;
		$total = conta('estoque_equipamento_retirada',"WHERE id AND data_solicitacao>='$data1' 
					 AND data_solicitacao<='$data2' AND status='Baixado'");
		$leitura = read('estoque_equipamento_retirada',"WHERE id AND data_solicitacao>='$data1' 
					 AND data_solicitacao<='$data2' AND status='Baixado' ORDER BY data_solicitacao DESC");
	}
...
<div class="content-wrapper">
	...
	<section class="content">
		...
		<div class="box-body table-responsive">
			...
			foreach($leitura as $mostra):
		 	...
		 	echo '<tr>';
				...
				echo '<td>'.substr($mostra['observacao'],0,12).'</td>';
		...
		$link = 'painel.php?execute=suporte/estoque/equipamento-retirada&pag=';
	     pag('estoque_equipamento_retirada',"WHERE id ORDER BY data_solicitacao DESC", $maximo, $link, $pag);
