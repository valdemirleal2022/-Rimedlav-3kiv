...
<?php 
	...
	if ( isset( $_POST[ 'relatorio-pdf' ] ) ) {
		...
		echo 'window.open("painel.php?execute=suporte/relatorio/relatorio-funcionario-acidentes-pdf");';
		echo '</script>';
	}
	...
	$total = conta('funcionario_acidente_trabalho',"WHERE id AND data>='$data1' AND data<='$data2' ORDER BY data ASC ");
	$leitura = read('funcionario_acidente_trabalho',"WHERE id AND data>='$data1' AND data<='$data2' ORDER BY data DESC");
	...
	if($leitura){
		...
		foreach($leitura as $mostra):
		 	echo '<tr>';
				...
				echo '<td>'.substr($veiculo['nome'],0,15).'</td>';
		
				$tipoId = $mostra['acidente_tipo'];
				$tipo = mostra('funcionario_acidente_tipo',"WHERE id ='$acidenteId'");
				...
				echo '<td>'.substr($mostra['status_seguradora'],0,25).'</td>';
