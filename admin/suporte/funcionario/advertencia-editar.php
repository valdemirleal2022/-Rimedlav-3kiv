<?php 
		...
		if(!empty($_GET['advertenciaEditar'])){
			$advertenciaId = $_GET['advertenciaEditar'];
			$acao = "atualizar";
		}
		if(!empty($_GET['advertenciaVisualizar'])){
			$advertenciaId = $_GET['advertenciaVisualizar'];
			$acao = "visualizar";
		}
		if(!empty($_GET['advertenciaDeletar'])){
			$advertenciaId = $_GET['advertenciaDeletar'];
			$acao = "deletar";
		}
		if(!empty($_GET['advertenciaBaixar'])){
			$advertenciaId = $_GET['advertenciaBaixar'];
			$acao = "baixar";
		}
		...
		if(!empty($advertenciaId)){
			$readadvertencia = read('funcionario_advertencia',"WHERE id = '$advertenciaId'");
			if(!$readadvertencia){
				header('Location: painel.php?execute=suporte/naoencontrado');	
			}
			foreach($readadvertencia as $edit);
		}
 ...
 <div class="content-wrapper">
  <section class="content-header">
          ...
          <ol class="breadcrumb">
            ...
            <li><a href="painel.php?execute=suporte/funcionario/advertencia">Advertencia</a></li>
  ...
  <section class="content">
      <div class="box box-default">
		...
		<?php 
			...
			if(isset($_POST['atualizar'])){
				...
				$edit['observacao'] = htmlspecialchars(($_POST['observacao']));
				$edit['id_motivo'] = mysql_real_escape_string($_POST['id_motivo']);
				$edit['usuario'] = $_SESSION['autUser']['nome'];
				update('funcionario_advertencia',$edit,"id = '$advertenciaId'");	
			...
			if(isset($_POST['cadastrar'])){
				...
				if(in_array('',$edit)){
				    ...
				    create('funcionario_advertencia',$edit);
			...
			if(isset($_POST['deletar'])){
					$readDeleta = read('funcionario_advertencia',"WHERE id = '$advertenciaId'");
					if(!$readDeleta){
						echo '<div class="alert alert-warning">Desculpe, o registro não existe!</div><br />';
					}else{
						delete('funcionario_advertencia',"id = '$advertenciaId'");
	 ...
	 if(isset($_POST['pdf-comprovante'])){
			...
			$diretorio = "../uploads/funcionarios/advertencia/";
		   ...
		   <div class="form-group col-xs-12 col-md-2">  
            ...
            <label>Motivo</label>
                <select name="id_motivo" class="form-control" >
                    <option value="">Selecione uma advertencia</option>
                    <?php 
                        $readSuporte = read('funcionario_advertencia_motivo',"WHERE id");
                        if(!$readSuporte){
          ...
          <div class="box">
         	<div class="box-header">
				  ...
				  <div class="form-group pull-left">
					...
					<?php
						$pdf='../uploads/funcionarios/advertencia/'.$edit['id'].'.pdf';

						if(file_exists($pdf)){

							echo '<a href="../uploads/funcionarios/advertencia/'.$edit['id'].'.pdf" target="_blank">
