...
<?php 
	...
	if ( isset( $_POST[ 'relatorio-pdf' ] ) ) {
		...
		$_SESSION['fim']=$data2;
		$_SESSION['suspensao']=$suspensao;
		$_SESSION['status']=$status;
		
		echo '<script type="text/javascript">';
		echo 'window.open("painel.php?execute=suporte/relatorio/relatorio-funcionario-suspensao-pdf");';
		echo '</script>';
	}
	...
	if ( isset( $_POST[ 'relatorio-excel' ] ) ) {
		...
		header( 'Location: ../admin/suporte/relatorio/relatorio-funcionario-suspensao-excel.php' );
	}
	...
	$total = conta('funcionario_suspensao',"WHERE id AND data>='$data1' AND data<='$data2' ORDER BY data ASC ");
	$leitura = read('funcionario_suspensao',"WHERE id AND data>='$data1' AND data<='$data2' ORDER BY data DESC");
	...
	if($leitura){
		...
		foreach($leitura as $mostra):
		 	echo '<tr>';
				...
				$suspensaoId = $mostra['id_motivo'];
				$funcionario = mostra('funcionario',"WHERE id ='$funcionarioId '");
		
				echo '<td>'.substr($funcionario['nome'],0,15).'</td>';
		
				$suspensao = mostra('funcionario_suspensao_motivo',"WHERE id ='$suspensaoId'");
				echo '<td>'.$suspensao['nome'].'</td>';
				...
				<a href="painel.php?execute=suporte/funcionario/suspensao-editar&suspensaoVisualizar='.$mostra['id'].'">
				...
				echo '<td align="center">
				<a href="painel.php?execute=suporte/funcionario/suspensao-editar&suspensaoBaixar='.$mostra['id'].'">
				...
				// imprimir ordem
				echo '<td align="center">
						<a href="painel.php?execute=suporte/funcionario/suspensaoImprimir&suspensao='.$mostra['id'].'" target="_blank">
