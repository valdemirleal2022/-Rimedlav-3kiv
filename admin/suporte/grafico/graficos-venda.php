 ...
 <div class="content-wrapper">
          ...
          <ol class="breadcrumb">
            ...
            <li class="active">Painel</li>
              ...
              <?php  
				...
				$contrato00 = soma('contrato',"WHERE tipo='2' AND Month(aprovacao)='$mesano[0]' AND 
									Year(aprovacao)='$mesano[1]' AND situacao<>'4'" ,'valor_mensal');
				...
				$tvenda0 = soma('contrato',"WHERE tipo='2' AND aprovacao='$data'",'valor_mensal');
				if(!$tvenda0){
					$tvenda0=0;
				...
				$tvenda1 = soma('contrato',"WHERE tipo='2' AND aprovacao='$data'",'valor_mensal');
				if(!$tvenda1){
					$tvenda1=0;
				...
				$tvenda2 = soma('contrato',"WHERE tipo='2' AND aprovacao='$data'",'valor_mensal');
				if(!$tvenda2){
					$tvenda2=0;
				...
				$tvenda3 = soma('contrato',"WHERE tipo='2' AND aprovacao='$data'",'valor_mensal');
				if(!$tvenda3){
					$tvenda3=0;
				...
				$tvenda4 = soma('contrato',"WHERE tipo='2' AND aprovacao='$data'",'valor_mensal');
				if(!$tvenda4){
					$tvenda4=0;
				...
				$tvenda5 = soma('contrato',"WHERE tipo='2' AND aprovacao='$data'",'valor_mensal');
				if(!$tvenda5){
					$tvenda5=0;
				...
				$tvenda6 = soma('contrato',"WHERE tipo='2' AND aprovacao='$data'",'valor_mensal');
				if(!$tvenda6){
					$tvenda6=0;
				...
				$tvenda7 = soma('contrato',"WHERE tipo='2' AND aprovacao='$data'",'valor_mensal');
				if(!$tvenda7){
					$tvenda7=0;
  ...
  <div class="row">
   <div class="col-md-12">  
       ...
       Aprovação Mensal  : <?php echo $mesano[0].'/'.$mesano[1] .' - R$   '.converteValor($venda00);?>
           ...
           <?php 
			...
			if($leitura){
				foreach($leitura as $mostraInd):
					$nome= $mostraInd['nome'];
					$Id = $mostraInd['id'];
					$total = soma('contrato',"WHERE tipo='2' AND Month(aprovacao)='$mesano[0]' 
											  AND Year(aprovacao)='$mesano[1]' AND indicacao='$Id'",'valor_mensal');
					if($total<>0){
						if(empty($nome1)){
							  $nome1=$nome;
							  $total1=$total;
 		...
 		<?php 
			...
			if($leitura){
				foreach($leitura as $mostraVend):
					...
					$Id = $mostraVend['id'];
					$total = soma('contrato',"WHERE tipo='2' AND Month(aprovacao)='$mesano[0]' 
											  AND Year(aprovacao)='$mesano[1]' AND consultor='$Id'",'valor_mensal');
					if($total<>0){
						if(empty($nome1)){
							  $nome1=$nome;
							  $total1=$total;
           ...
           <div class="col-md-6">   
			...
			if($leitura){
				foreach($leitura as $mostraInseto):
					...
					$total=0;
					$leitura = read('contrato',"WHERE tipo='2' AND Month(aprovacao)='$mesano[0]' 
												AND  Year(aprovacao)='$mesano[1]'");
					foreach($leitura as $mostracontrato):
						$contratoId = $mostracontrato['id'];
						$totalIns= soma('contrato_coleta',"WHERE tipo_coleta='$insetoId' AND id_contrato='$contratoId'",'valor_mensal');
     	...
     	<script type="text/javascript">
			  ...
			  function drawChart() {
				...
				var options = {
				   is3D: true,
				   legend:{position: 'right', textStyle: {color: 'black', fontSize: 8}},
				   title: 'Aprovação por Classificacao'
				};
