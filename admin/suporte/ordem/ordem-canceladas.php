<?php 
	...
	if ( isset( $_POST[ 'relatorio-pdf' ] ) ) {
		...
		echo 'window.open("painel.php?execute=suporte/relatorio/relatorio-ordem-canceladas-pdf");';
		echo '</script>';
	}
	...
	if ( isset( $_POST[ 'relatorio-excel' ] ) ) {
		...
		$data1 = $_POST[ 'inicio' ];
		$data2 = $_POST[ 'fim' ];
		header( 'Location: ../admin/suporte/relatorio/relatorio-ordem-canceladas-excel.php' );
	}
	...
	$total = conta('contrato_ordem',"WHERE id AND status='15'");
	$leitura = read('contrato_ordem',"WHERE id AND status='15' 
						ORDER BY interacao DESC" );

	if(isset($_POST['pesquisar'])){
		$data1 = $_POST[ 'inicio' ];
		$data2 = $_POST[ 'fim' ];
		$total = conta('contrato_ordem',"WHERE id AND data>='$data1' AND data<='$data2' AND status='15'");
		$leitura = read('contrato_ordem',"WHERE id AND data>='$data1' AND data<='$data2' AND status='15'
									ORDER BY data ASC");
	}
