<?php 
	...
	if(!empty($_SESSION['autConsultor'])){
...
<html><head>
    ...
    <title>Clean Ambiental| Suporte ao Consultor</title>
			...
			<?php 
                if(isset($_POST['login'])){
                    ...
                    if(!$email || !email($email)){
                        ...
                        $autEmail = $email;
                        $autSenha = $senha;	
                        $leitura = read('contrato_consultor',"WHERE email = '$autEmail'");
                        if($leitura){
                            foreach($leitura as $autConsultor);
							
                           if($autSenha == $autConsultor['senha']){
									
						if($autConsultor['status'] == '1'){ // verificado status - ativo ou inativo

                                $_SESSION['autConsultor'] = $autConsultor;	// criar usuario	
								
								 // atualizada geolizacao
								 $usuarioId= $autConsultor['id'] ;
								 $cad['latitude']=$latitude ;
								 $cad['longitude']=$longitude;
								 update('contrato_consultor', $cad, "id = '$autConsultor'");
									
								// registro login e geolizacao
								$usu['id_consultor']= $autConsultor['id'] ;
