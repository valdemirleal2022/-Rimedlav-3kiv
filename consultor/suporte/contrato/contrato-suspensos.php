 ...
 <?php 
	...
	$valor_total = soma('contrato',"WHERE id AND tipo='2' AND status='6' AND consultor='$consultorId'",'valor_mensal');
	$total = conta('contrato',"WHERE id AND tipo='2' AND status='6' AND consultor='$consultorId'");
	$leitura = read('contrato',"WHERE id AND tipo='2' AND status='6' AND consultor='$consultorId' ORDER BY data_suspensao DESC");
