...
<section class="content">
  ...
  <div class="row">
   <div class="col-md-12">  
	...
	$valor_total = soma('cadastro_visita',"WHERE id AND status='4' AND consultor='$consultorId'",'orc_valor');
 	$total = conta('cadastro_visita',"WHERE id AND status='4' AND consultor='$consultorId'");
	
	$leitura = read('cadastro_visita',"WHERE id AND status='4' AND consultor='$consultorId' ORDER BY orc_solicitacao DESC LIMIT $inicio,$maximo");
	if($leitura){
		...
		foreach($leitura as $mostra):
		 	...
		 	echo '<tr>';
				...
				echo '<td align="center">
						<a href="painel.php?execute=suporte/orcamento/orcamento-editar&orcamentoVisualizar='.$mostra['id'].'">
			  				<img src="../admin/ico/visualizar.png" alt="Visualizar" title="Visualizar Orçamento" />
              			</a>
				      </td>';
		 ...
		 echo '</table>';
		 $link = 'painel.php?execute=suporte/orcamento/orcamento-aprovados&pag=';
		 pag('cadastro_visita',"WHERE status='4' AND consultor='$consultorId' ORDER BY orc_solicitacao DESC", $maximo, $link, $pag);
