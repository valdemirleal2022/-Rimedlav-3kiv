...
<section class="content">
  ...
  <div class="row">
   <div class="col-md-12">  
	...
	$valor_total = soma('cadastro_visita',"WHERE id AND status='18' AND consultor='$consultorId'",'orc_valor');
 	$total = conta('cadastro_visita',"WHERE id AND status='18' AND consultor='$consultorId'");
	
	$leitura = read('cadastro_visita',"WHERE id AND status='18' AND consultor='$consultorId' ORDER BY orc_solicitacao DESC LIMIT $inicio,$maximo");
	if($leitura){
		...
		foreach($leitura as $mostra):
		 	...
		 	echo '<tr>';
				...
				$motivoId = $mostra['motivo_cancelamento'];
				$motivo = mostra('cadastro_visita_motivo_cancelamento',"WHERE id ='$motivoId'");
				echo '<td>'.$motivo['nome'].'</td>';
				
				
				echo '<td align="center">
					<a href="painel.php?execute=suporte/orcamento/orcamento-editar&orcamentoEditar='.$mostra['id'].'">
					<img src="../admin/ico/visualizar.png" alt="Visualizar" title="Visualizar" />
					</a>
					</td>';
