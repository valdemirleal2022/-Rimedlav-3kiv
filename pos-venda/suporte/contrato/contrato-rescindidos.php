 ...
 <?php 
	...
	$valor_total = soma('contrato',"WHERE id AND tipo='2' AND status='7' AND pos_venda='$pos_vendaId' ",'valor_mensal');
	$total = conta('contrato',"WHERE id AND tipo='2' AND status='7' AND pos_venda='$pos_vendaId' ");
	$leitura = read('contrato',"WHERE id AND tipo='2' AND status='7' AND pos_venda='$pos_vendaId'  ORDER BY data_rescisao DESC");
	...
	if($leitura){
		...
		foreach($leitura as $mostra):
		 	echo '<tr>';
				...
				echo '<td align="left">'.substr($cliente['bairro'],0,35).'</td>';
